...
const MapSidebar: React.FC<SidebarProps> = ({
  ...
  const imageSize = sidebarWidth <= 250 ? 50 : sidebarWidth >= 300 ? 100 : 75;
  const columns = sidebarWidth >= 400? 4 :  sidebarWidth >= 300 ? 3 : 2;
  ...
  return (
    <Sidebar
      ...
      className={`relative bg-1-light dark:bg-1-dark border-r border-border-light dark:border-border-dark duration-200 ease-in-out transition-all ${
        isTransitioning ? "ease-out duration-200" : "duration-0"
      }`}>
        ...
        buildMap={(mapData) => {
          ...
          setCharacters(
            mapData.characters.map((char: Character) => ({
              ...
              imagePath: char.class ? `/characters/${char.class.toLowerCase()}.png` : "/characters/fighter.png",
            }))
          );
        }}
      ...
      <div className="h-auto">
        <div className="relative flex space-x-2 mb-4 px-4">
          {[
            ...
            <button
              ...
              className={`flex flex-grow items-center p-2 rounded-md text-sm transition-all ease-out duration-200 ${
                activeTab === tab ? "bg-tetriary-light dark:bg-tetriary-dark text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-300"
              }`}>
              <Icon className="w-5 h-5 flex-shrink-0 transition-all ease-out duration-200" />
              <span
                className={`ml-2 transition-opacity ease-out duration-200 absolute ${
                  sidebarWidth > 270 && (activeTab === tab || sidebarWidth > 420) ? "opacity-100 relative" : "opacity-0 hidden"
                }`}>
                {label}
              </span>
            </button>
        ...
        <div className="">
          <div className="overflow-y-auto pl-4 pr-2 mr-2" style={{ height: "calc(100svh - 20rem)" }}>
            {activeTab === "tiles" && (
                ...
                {champions.map((char) => (
                  <button
                    ...
                    className={`p-2 border border-border-light dark:border-border-dark rounded-md ${
                      characters.some((temp) => temp.name === char.name) && "opacity-50 cursor-not-allowed"
                    }`}
                    disabled={characters.some((temp) => temp.name === char.name)}>
                    <NextImage alt={char.name} src={`/characters/${char.class.toLowerCase()}.png`} className="w-full" width={imageSize} height={imageSize} draggable="false" />
              ...
              <div
                ...
                {structureImages.map((structure) => (
                  <button
                    ...
                    className="p-2 border border-border-light dark:border-border-dark rounded-md">
                    <NextImage src={structure.url} alt={structure.name} width={imageSize} height={imageSize} className="w-full" draggable="false" />
            ...
            {activeTab === "settings" && (
              <div className="space-y-2">
                <div className="border rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer bg-white dark:bg-gray-800">
                  <button onClick={() => setIsSavePopupOpen(true)} className="w-full rounded-lg hover:bg-blue-200 p-4 dark:hover:bg-blue-500">
                    Save Map
                  </button>
                </div>
                <div className="border rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer bg-white dark:bg-gray-800">
                  <button onClick={() => setIsLoadPopupOpen(true)} className="w-full rounded-lg hover:bg-red-200 p-4 dark:hover:bg-red-400">
                    Load Map
                  </button>
                </div>
              </div>
      ...
      {sidebarWidth > 50 && (
        <div className="absolute left-0 bottom-0 w-full bg-gray-300 dark:bg-gray-700 overflow-hidden">
          <div className="p-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2 rounded-md transition-all ease-out duration-200
                        ${selectionMode === mode ? "bg-tetriary-dark text-white shadow-md shadow-blue-500/50" : "bg-gray-700 text-gray-300 hover:bg-gray-600"}`}>
