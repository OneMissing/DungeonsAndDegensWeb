			...
			{skills.map((skill, index) => (
				<div
					...
					<div className="flex gap-2">
						<CirclePlus
							color={`${character[skill.toLowerCase().replace(/ /g, "_") as keyof Character] === 20 ? "gray" : "green"}`}
							onClick={() => updateSkill(skill, 1)}
						/>
						<CircleMinus
							color={`${character[skill.toLowerCase().replace(/ /g, "_") as keyof Character] === -20 ? "gray" : "red"}`}
							onClick={() => updateSkill(skill, -1)}
						/>
					</div>
				</div>
