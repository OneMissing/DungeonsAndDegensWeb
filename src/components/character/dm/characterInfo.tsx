...
import { Character, CharacterStat, Classes, Races, CharacterAttribute } from "@/lib/tools/types";
...
const CharacterInfo = ({ characterId, className }: CharacterInfoProps) => {
	...
	const [attributes, setAttributes] = useState<CharacterAttribute | null>(null);
	...
	const handleDoubleClick = (field: keyof Character | keyof CharacterAttribute | keyof CharacterStat) => {
		if (!character || !attributes) return;
		if (field in character) {
			setFieldValue(character[field as keyof Character]?.toString() ?? "");
		} else if (field in attributes) {
			setFieldValue(attributes[field as keyof CharacterAttribute]?.toString() ?? "");
		}
	...
	const handleBlur = async () => {
		...
		if (editingField in character!) {
			setCharacter((prev) => (prev ? { ...prev, [editingField]: updatedValue } : prev));
			await updateCharacter(character!.id, {
				[editingField]: updatedValue,
			});
		} else if (editingField in attributes!) {
			setAttributes((prev) => (prev ? { ...prev, [editingField]: updatedValue } : prev));
			await supabase
				.from("character_attrib")
				.update({ [editingField]: updatedValue })
				.eq("character_id", characterId);
		} else if (editingField in stats!) {
			setStats((prev) => (prev ? { ...prev, [editingField]: updatedValue } : prev));
			await supabase
	...
	return (
        ...
        <h2 className="text-4xl font-bold text-center" onDoubleClick={() => handleDoubleClick("name")}>
			...
			<div className="text-center">
				...
				<p>
					AC:{" "}
					<span onDoubleClick={() => handleDoubleClick("ac")} className="inline-block w-12 text-center">
						{editingField === "ac" ? (
							<input
								type="text"
								pattern="[0-9]*"
			...
			<ul className="grid grid-cols-2 gap-4 text-gray-700">
				{attributes &&
					(["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"] as (keyof CharacterAttribute)[]).map((attr) => (
