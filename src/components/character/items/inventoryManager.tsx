...
const ContextMenu: React.FC<ContextMenuProps> = ({ item, position, onClose, onAction }) => {
	...
	return (
		<div
			...
			<div className={`grid ${uniqueInstanceTypes.includes(item.type) ? "grid-rows-6" : "grid-rows-8"}`}>
				...
				<Divider className="my-2" />
				{!uniqueInstanceTypes.includes(item.type) && <Slider min={1} max={99} value={addSliderValue} onChange={setAddSliderValue} />}
				{!uniqueInstanceTypes.includes(item.type) && (
...
const DroppableTile: React.FC<{
	...
	onItemRemoved: (itemId: string) => void;
	onQuantityChanged: (itemId: string, newQuantity: number) => void;
}> = ({ tile, moveItem, grid, character_id, onItemRemoved, onQuantityChanged }) => {
	const [, drop] = useDrop({
	...
	return (
		<div
			...
			{tile.isTrash ? (
				...
				tile.item && <DraggableItem character_id={character_id} item={tile.item} onItemRemoved={onItemRemoved} onQuantityChanged={onQuantityChanged} itemEffects={[]} />
			)}
		</div>
	);
};
...
const Inventory: React.FC<{
	...
	itemEffectsMap: Record<string, ItemEffect[]>;
	character: any;
